...
urlInfo = [('/', StartPage),
	  ...
	  (BuildURL("dir_visit", URLForEntryType("topic")), EnterEntryPage),
	  ...
	  (BuildURL("dir_curate", "url_bulkcreatetags"), CurateBulkCreateTagsPage),
	  # guiding
	  (BuildURL("dir_guide"), ReviewResourcesPage),
	  (BuildURL("dir_guide", extraSlash=True), ReviewResourcesPage),
	  (BuildURL("dir_guide", "url_topics"), ReviewTopicsPage),
	  ...
	  ('/%s/%s_%s' % (DIRS["dir_manage"], URLS["url_questions"], URLForQuestionRefersTo("topic")), ManageRakontuQuestionsPage),
